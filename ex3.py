# A new loss function for discriminative tagging
import torch
...
class BiLSTM_CRF(nn.Module):
    ...
    def score_diff_loss(self, sentence, tags):
        # Use the difference between the Viterbi path score and the score of
        # the gold-standard path to compute diff-square loss function
        self.hidden = self.init_hidden()
        feats = self._get_lstm_features(sentence)
        gold_score = self._score_sentence(feats, tags)
        viterbi_score, _ = self._viterbi_decode(feats)
        return torch.pow(viterbi_score - gold_score, 2)
...
for epoch in range(300): # again, normally you would NOT do 300 epochs, it is toy data
    for sentence, tags in training_data:
        ...
        loss = model.score_diff_loss(sentence_in, targets)
...
precheck_sent = prepare_sequence(training_data[0][0], word_to_ix)
print(model(precheck_sent))
# It seems that our new loss function may make some mistakes
# Maybe the argmax function retards the training progress
